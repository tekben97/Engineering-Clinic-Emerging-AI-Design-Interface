...
def run_image(image, src, inf_size, obj_conf_thr, iou_thr, conv_layor):
    ...
    new_dir = generate_feature_maps(opened_image, conv_layor)
...
with gr.Blocks(title="YOLO7 Interface",theme=gr.themes.Base()) as demo:
    ...
    with gr.Row() as settings:
        inf_size = gr.Slider(label='Inference Size (pixels)',info="Choose a whole number for the inference size",minimum=1,maximum=1280,step=1,value=640)
        obj_conf_thr = gr.Slider(label='Object Confidence Threshold',info="The amount of confidence needed for the model to show its guess. Any guess that is below the amount is not shown in the output.",minimum=0.01,maximum=1.00,step=0.01,value=0.25)
        iou_thr = gr.Slider(label='IOU threshold for NMS',info="The intersect over union threshold",minimum=0.01,maximum=1.00,step=0.01,value=0.45) 
    ...
    im_com_but.click(run_image, inputs=[im_com_input, source_type, inf_size, obj_conf_thr, iou_thr, conv_layor], outputs=[im_output, im_conv_output, im_smooth_output])
    vid_com_but.click(run_video, inputs=[vid_com_input, source_type, inf_size, obj_conf_thr, iou_thr], outputs=[vid_output])
    im_web_but.click(run_image, inputs=[im_web_input, source_type, inf_size, obj_conf_thr, iou_thr, conv_layor], outputs=[im_output, im_conv_output, im_smooth_output])
